 ...
 * @version $Id: EventParser.java,v 1.5 2014/06/20 10:01:26 justb Exp $
 */
public class EventParser {
    ...
    public static Event parse(Reader aReader) throws IOException {
        ...
        // EOF reached before starting '<' found
        if (nextInt == -1) {
            throw new IOException("No XML start tag found");
        }

        // Then find end-tag ('>'), appending all chars to preparsed string.
        do {
            nextInt = aReader.read();
            if (nextInt == -1) {
                // EOF reached before ending '>' found
                throw new IOException("No XML end tag found");
            }
...
/*
  ...
  * Revision 1.5  2014/06/20 10:01:26  justb
  * fix for ticket [#12] detect missing XML start/end tags in EventParser.parse(Reader) - reformat code
  *
  * Revision 1.4  2014/06/20 09:55:33  justb
  * fix for ticket [#12] detect missing XML start/end tags in EventParser.parse(Reader)
  *
